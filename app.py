...
# Change the password here if appliable
...
def register():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'fname' in request.form and 'mname' in request.form and 'lname' in request.form and 'language' in request.form:
        ...
        if user:
            ...
            cursor.execute('INSERT INTO user (username,password,fName,mName,lName) VALUES (% s, % s, % s, % s, % s)',
                           (username, password, fname, mname, lname, ))
            mysql.connection.commit()
            cursor.execute(
                'INSERT INTO User_Genre VALUES (% s, % s)', (username, fgenre, ))
            mysql.connection.commit()
...
def addmovie():  # Need to add in genre, platform, user and platform ratings
    ...
    if request.method == 'POST' and 'movieTitle' in request.form and 'genre' in request.form and 'platform' in request.form and 'summary' in request.form and 'movieTime' in request.form and 'director' in request.form and 'userRating' in request.form and 'platformRating' in request.form and 'moviePoster' in request.form:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('INSERT INTO movie (name,summary,director,duration,moviePoster) VALUES (% s, % s, % s, % s, % s)',
                       (movieTitle, summary, director, movieTime, moviePoster,))
        mysql.connection.commit()
        cursor.execute(
            'INSERT INTO Movie_Genre (movieID,genreName) VALUE ((SELECT id FROM movie where name = %s LIMIT 1),%s)', (movieTitle, genre, ))
        mysql.connection.commit()
        cursor.execute(
            'INSERT INTO Watch_On (movieID,platformName) VALUE ((SELECT id FROM movie where name = %s LIMIT 1),%s)', (movieTitle, platform, ))
        mysql.connection.commit()
        cursor.execute('INSERT INTO Rating (platformRating,platformName,username,movieID) VALUE (%s,%s,(SELECT ownerUsername FROM platform where name =%s ),(SELECT id FROM movie where name = %s LIMIT 1))',
                       (platformRating, platform, platform, movieTitle, ))
        mysql.connection.commit()
...
def profile():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'fname' in request.form and 'mname' in request.form and 'lname' in request.form and 'language' in request.form:
        ...
        if user:
            ...
            cursor.execute('UPDATE user SET  username =% s, password =% s, fname =% s, mname =% s, lname =% s WHERE username =% s',
                           (username, password, fname, mname, lname, (session['username'], ), ))
            mysql.connection.commit()
            cursor.execute('UPDATE User_Genre set username =% s, favoriteGenre = %s WHERE username=%s',
                           (username, fgenre, (session['username'],),))
...
def editmovie():  # Need to add in genre, platform, user and platform ratings
    ...
    if request.method == 'POST' and 'movieTitle' in request.form and 'genre' in request.form and 'platform' in request.form and 'summary' in request.form and 'movieTime' in request.form and 'director' in request.form and 'userRating' in request.form and 'platformRating' in request.form and 'moviePoster' in request.form:
        ...
        moviePoster = request.form['moviePoster']
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('UPDATE movie SET  name =% s, summary =% s, director =% s, duration =% s, moviePoster =% s WHERE name = %s',
                       (movieTitle, summary, director, movieTime, moviePoster, movieTitle, ))
        mysql.connection.commit()
        cursor.execute(
            'SELECT id FROM movie where name = %s Limit 1', (movieTitle,))
        movieID = cursor.fetchone()
        movieID = movieID['id']
        cursor.execute(
            'UPDATE Movie_Genre SET genreName = %s WHERE movieID = % s', (genre, movieID,))
        cursor.execute(
            'UPDATE Watch_On SET platformName = %s WHERE movieID = % s', (platform, movieID,))
        cursor.execute('UPDATE Rating SET platformRating = %s, platformName = %s, username = %s  WHERE movieID = %s',
                       (platformRating, platform, session['username'], movieID,))
        cursor.execute(
            'SELECT id FROM Rating where movieID = %s Limit 1', (movieID,))
        ratingID = cursor.fetchone()
        ratingID = ratingID['id']
        cursor.execute(
            'UPDATE User_Rating set userRating=%s where ratingID = %s', (userRating, ratingID, ))
        msg = 'You have successfully added your movie!'
    elif request.method == 'POST':
        msg = 'Please fill out the form !'
...
def searchmovie():
    if request.method == "POST":
        try:
            ...
            """
            cursor.execute(
                "SELECT * FROM movie where movieTitle = %s", (movie,))
